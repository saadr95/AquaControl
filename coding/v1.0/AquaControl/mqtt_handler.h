...
//
// Broker/auth is controlled entirely by the MQTT_USE_TLS/MQTT_USERNAME
// block in config.h — see the comment there for the plan to migrate off
// today's public broker.
//
...
void initMQTT() {
#if MQTT_USE_TLS
  // Same tradeoff already accepted in ota.h: skips certificate validation
  // for simplicity. A pinned/real cert store would be the more correct
  // long-term fix, but this is no worse than what OTA already does.
